...
export class MasteryService {
  ...
  public updateMasteryOnAttempt(
    ...
    // A review only counts as "delayed" (and only advances the spaced interval) when the concept was actually due.
    const wasDue = !existing.nextReviewDue || new Date(existing.nextReviewDue).getTime() <= Date.now();
    const hoursSinceLast = existing.lastPracticed ? (Date.now() - new Date(existing.lastPracticed).getTime()) / 3600000 : 0;
    ...
    if (isCorrect) {
      ...
      // Only advance the interval when the review was due: several correct answers in one sitting count once.
      if (wasDue) {
