...
import { generateSubjectAwareFallbackQuestion } from './server/services/questionUtils.ts';
...
app.post('/api/curriculum/subjects', (req, res) => {
  ...
  const newSubject: Subject = {
    ...
    name,
    code: code || '',
    description: description || `School syllabus for ${name}`,
...
app.post('/api/learn/submit-attempt', async (req, res) => {
  try {
    ...
    if (question.type === 'MULTIPLE_CHOICE') {
      ...
      const tolerance = question.tolerance !== undefined ? question.tolerance : 0.05;
...
app.get('/api/planner/today', (req, res) => {
  const minutes = parseInt(req.query.targetMinutes as string) || 90;
  const plan = plannerService.generateDailyPlan(minutes);
  res.json(plan);
});
...
app.post('/api/tests/generate', async (req, res) => {
  try {
    ...
    for (const [idx, concept] of selectedConcepts.entries()) {
...
app.post('/api/tests/submit', async (req, res) => {
  try {
    const { testId, subjectId, type, answers } = req.body;
    ...
    for (const ans of answers) {
      ...
      if (q.type === 'MULTIPLE_CHOICE') {
        ...
        const diff = Math.abs(parseFloat(ans.studentAnswer) - parseFloat(q.correctAnswer));
        isCorrect = !isNaN(diff) && diff <= (q.tolerance || 0.05);
      } else {
        isCorrect = ans.studentAnswer.trim().length > 15;
      }

      if (concept) {
        masteryService.updateMasteryOnAttempt(concept.id, isCorrect, q.category);
        if (isCorrect) {
          strongConcepts.add(concept.name);
        } else {
          weakConcepts.add(concept.name);
          const mistake = await mistakeService.recordMistake(q, concept, ans.studentAnswer);
    ...
    const testRecord: TestRecord = {
      ...
      title: `${type === 'MOCK_EXAM' ? 'Full Term Mock Exam' : 'Assessment Test'}`,
      ...
      durationMinutes: Math.round(totalQuestions * 2.5),
...
async function setupServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Academic Mastery server listening on http://0.0.0.0:${PORT}`);
  });
}
